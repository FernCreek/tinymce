...
const render = function (editor, theme, args) {
    ...
    items: [
      {
        ...
        classes: 'top-part',
        items: [
          Settings.hasMenubar(editor) === false ? null : { type: 'menubar', border: '0 0 0 0', items: Menubar.createMenuButtons(editor) },
  ...
  if (Settings.hasStatusbar(editor)) {
    ...
    panel.add({
      type: 'panel', name: 'statusbar', classes: 'statusbar', layout: 'flow', border: '0 0 0 0', ariaRoot: true, items: [
