...
const countTableColumns = ($table) => {
  const countRowCols = ($row) => !!$row ? $row.find('td').reduce((cols, row) => cols + $(row).prop('colSpan'), 0) : 0;
  return !!$table ? $table.find('tr').reduce((currCol, row) => Math.max(countRowCols($(row)), currCol), 0) : 0;
};
