	...
	*/
	...
	tinymce.create('tinymce.plugins.SeapineTablePlugin', {
		...
		getTableAlignment: function ($table) {
			...
			if ($table && $table.length === 1) {
				...
				if (floatValue === 'left') {
					...
					align = 'right';
				} else if (floatValue === 'none' && $table[0].style['margin-left'] === 'auto' &&
						$table[0].style['margin-right'] === 'auto') {
					align = 'center';
				} else {
		...
		adjustTableWidth: function ($table, tableWidth) {
			var cssWidth, oldWidth, ratio, self = this;
			if ($table && $table.length === 1) {
					...
					if (oldWidth !== tableWidth) {
						...
						$table.find('td').each(function () {
							...
							if ($cell[0].style.width) {
								$cell.css('width', Math.round(self.getWidthFromPxString($cell[0].style.width) * ratio) + 'px');
